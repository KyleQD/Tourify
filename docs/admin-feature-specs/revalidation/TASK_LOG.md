...
- **Decision trail:** Site-map template, export, and version reads authenticated independently or used the broad compatibility Admin wrapper even though canonical acting-context and `logistics.view` enforcement already exists. Registry labels alone could not prove those handlers used the declared boundary.
- **Change:** Migrated the three handlers to `withAdminCapability('logistics.view')`, retained site-map owner/collaborator export/read checks, and removed duplicate handler-level authentication. Added a focused source contract that rejects reintroduction of `withAdminAuth` or direct `auth.getUser()` in those handlers. Reclassified the exact routes and reduced the no-growth ceiling from 117 to 114.
- **Boundary:** Public map templates remain intentionally discoverable under their existing public-or-creator RLS rule. This batch does not claim organization-key convergence for deferred equipment catalog data, command idempotency, staging RLS proof, or completion of the other 114 legacy handlers.
- **Verify:** Exact 288-route parity passes at 114/114; nine focused route/capability tests pass; the complete Admin suite passes 228 files and 3,206 tests with one live-security file and two environment-dependent tests still skipped; touched lint and scoped TypeScript pass.
