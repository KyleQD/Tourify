...
### 2026-08-27 — `SEC-104` (in progress: exact inventory and first logistics convergence wave)

- **Spec:** `docs/admin-feature-specs/01_Platform_Tenancy_RBAC_and_Audit.md` — classify every Admin route and migrate high-risk finance, ticketing, logistics, and tour commands first while CI rejects any unclassified addition.
- **Decision trail:** Route-level capability labels alone did not state tenant target, privileged-client use, audience, lifecycle disposition, owning end-to-end workflow, audit event, or test evidence. Provider controls and a test route also remained indistinguishable from organization Admin surfaces.
- **Change:** Expanded all 288 route-method contracts with tenant target, acting-context source, capability/mode, schema IDs, service-role posture, idempotency, audit event, workflow IDs, test IDs, visibility, disposition, owner, and legacy state. Provider ingestion controls are explicitly platform-internal and the test route is marked for retirement. Migrated Admin logistics communications and staff-channel privileged access into named, organization-bound service-role jobs. Revalidated five logistics handlers already enforcing `withAdminCapability` and lowered the no-growth ceiling from 122 to 117.
- **Boundary:** Classification and schema identifiers do not prove runtime validation, audit emission, recovery, or denial behavior. The 117 remaining legacy handlers and every `legacy_missing` idempotency/audit declaration remain open; no staging or production database was contacted.
- **Verify:** Exact 288-route parity passes at 117/117 legacy ceiling; the capability matrix passes six focused tests; service-role job tests and touched lint pass; the complete Admin suite passes 227 files and 3,202 tests with one live-security file and two environment-dependent tests still skipped.
- **Blocker:** Continue domain-by-domain handler convergence, beginning with remaining logistics and hiring routes; replace stable schema IDs with enforced validators, add command idempotency and audit emission, reduce the legacy ceiling with every batch, and run direct two-organization denial/replay tests on dedicated staging before completion.

### 2026-08-28 — `SEC-104` (in progress: site-map read boundary)

- **Spec:** `docs/admin-feature-specs/01_Platform_Tenancy_RBAC_and_Audit.md` — migrate existing Admin endpoints to trusted acting context and operation-specific capabilities while lowering the enforced legacy ceiling.
- **Decision trail:** Site-map template, export, version, and activity handlers authenticated independently or used the broad compatibility Admin wrapper even though canonical acting-context enforcement already exists. The first wrapper pass also exposed a multi-organization composition flaw: active-org capability and site-map access could succeed against different organizations for the same multi-org user.
- **Change:** Migrated template, export, version, activity, and collaborator handlers to canonical capability wrappers. Linked site-map access resolves every supplied legacy event/tour parent, requires one unambiguous organization, compares it to `admin.orgId` before owner/collaborator checks, and hides mismatched or unscoped records. Activity writes accept only presence or a validated element-status command, stamp the authenticated actor and route map, verify the element belongs to that map, bound pagination, and declare audit per method. Collaborator reads/removals share the same active-org boundary; removal accepts one strict UUID and retains the existing owner-only rule. The no-growth ceiling fell from 117 to 112.
- **Boundary:** Public map templates remain intentionally discoverable under their existing public-or-creator RLS rule. Unlinked site maps now fail closed in these organization-bound handlers; the legacy detail/create and collaborator-share routes still need org-key/invitation convergence. This batch does not claim organization-key convergence for deferred equipment catalog data, idempotency, staging RLS proof, or completion of the other 112 legacy handlers.
- **Verify:** Exact 288-route parity passes at 112/112; focused capability, active-org, tamper, actor-binding, child-ownership, pagination, and collaborator-input tests pass; the complete Admin suite passes 231 files and 3,223 tests with one live-security file and two environment-dependent tests still skipped; touched lint and scoped TypeScript pass under Node 20. Live Supabase cases remain unverified.
- **Blocker:** Continue with site-map mutations and child routes only after their active migration/RLS ownership is reconciled; keep equipment catalog legacy until a non-invented organization owner exists; then run direct Org A/Org B/revoked/unauthenticated cases on dedicated staging.

### 2026-09-03 — `SEC-104` (in progress: site-map element command boundary)

- **Spec:** `docs/admin-feature-specs/01_Platform_Tenancy_RBAC_and_Audit.md` — migrate child mutations to the trusted acting organization, bind actors server-side, validate commands, preserve audit evidence, and lower the enforced legacy ceiling.
- **Decision trail:** Element collection autosave accepted arbitrary objects, could upsert a client-supplied ID already attached to another map, ignored sync-delete errors, and could partially upsert before delete. Element detail GET/PUT/DELETE checked authentication only and relied on child RLS without proving parent map access or selected organization. Parent resolution also queried legacy `events` although the locked canonical model and generated FK target `events_v2`. Existing child policies omitted owner and organization-capability write paths, while archived logistics helpers were referenced by runtime code but absent from the active migration chain.
- **Change:** Both element routes now use `logistics.view` for reads and `logistics.manage` for writes, require one active-organization-bound parent map, validate UUID route/input identities and bounded geometry/color/JSON, reject client tenant/actor fields and cross-map ID reuse, preserve existing JSON properties on partial updates, return correlated stable errors, and scope every child query by map plus element. Canvas IDs remain UUIDs in fallback browsers and persisted colors remain database-compatible. Added an actor-bound `can_logistics`, conflict-safe canonical parent resolution, expiry-aware collaborator access, explicit element/activity policies, prospective database constraints, and a `SECURITY INVOKER` atomic full-canvas sync RPC; mutation triggers retain the authenticated actor. The migration validator now distinguishes inert stored-routine DML from migration-time deletion while still rejecting top-level destructive SQL.
- **SQL:** Added `20260903090000_site_map_element_security_and_atomic_sync.sql` with a truthful `planned` validation manifest. No hosted Supabase environment was contacted and no production data changed.
- **Verify:** 30 focused command/capability/RLS tests pass; exact 288-route parity passes at 110/110; touched ESLint and scoped TypeScript pass; all migration validation and service-role checks pass; the full Admin suite passes 233 files and 3,244 tests with one live-security file and two environment-dependent tests skipped. The migration applied successfully to a disposable PostgreSQL fixture; two-row create, same-payload replacement, cross-map collision rollback, forged tenant-field and invalid-geometry rollback, expired-collaborator denial, and cross-actor read/write denial all passed.
- **Blocker:** Apply the immutable migration to the dedicated Supabase staging project and run direct Org A/Org B/revoked/stale/anonymous plus activity-audit tests. The broader logistics contract still correctly fails on three capabilities whose only DDL is archived (`logistics_team_communications_scope`, `logistics_ops_foundation`, and `site_map_zone_ownership`); promote reviewed replacements rather than pointing tests at non-applied archive files.
