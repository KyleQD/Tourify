...
- **Change:** Migrated template, export, version, and activity handlers to canonical capability wrappers. Linked site-map access now resolves every supplied legacy event/tour parent, requires one unambiguous organization, compares it to `admin.orgId` before owner/collaborator checks, and hides mismatched or unscoped records. Activity writes accept only presence or a validated element-status command, stamp the authenticated actor and route map, verify the element belongs to that map, bound pagination, and declare audit per method. The no-growth ceiling fell from 117 to 113.
- **Boundary:** Public map templates remain intentionally discoverable under their existing public-or-creator RLS rule. Unlinked site maps now fail closed in these organization-bound handlers; the legacy detail/create routes still need an active org-key migration. This batch does not claim organization-key convergence for deferred equipment catalog data, idempotency, staging RLS proof, or completion of the other 113 legacy handlers.
- **Verify:** Exact 288-route parity passes at 113/113; focused capability, active-org, tamper, actor-binding, child-ownership, and pagination tests pass; the complete Admin suite passes 230 files and 3,218 tests with one live-security file and two environment-dependent tests still skipped; touched lint and scoped TypeScript pass under Node 20. Live Supabase cases remain unverified.
