...
| `REL-103` | blocked | TypeScript-AST CI now enforces exact 288 route and route-method parity, rejects duplicates/stale entries, and materializes tenant target, acting context, capability mode, schema IDs, service-role posture, idempotency, audit event, workflow, tests, visibility, disposition, and owner; the ceiling has fallen to 117 legacy routes, which still require runtime schema/idempotency/audit convergence |
...
| `SEC-104` | in_progress | All 288 Admin routes are classified; five additional logistics routes are capability-gated, 117 legacy routes remain, and provider/internal/test dispositions are explicit. Admin tests and route parity pass; concrete runtime schemas, idempotency, audit emission, remaining handler convergence, and independent denial evidence remain. |
