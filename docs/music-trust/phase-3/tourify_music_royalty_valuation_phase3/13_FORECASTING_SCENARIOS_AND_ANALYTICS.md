# Forecasting, Scenarios, and Analytics

## Forecast dimensions

- existing catalog natural decay;
- recent release momentum;
- platform, territory, and product mix;
- seasonality;
- artist social and ticket conversion as supplemental signals;
- release schedule;
- sync or direct-license pipeline only when probability-adjusted;
- term expirations and ownership reversion;
- recoupment exhaustion;
- distributor or administrator changes;
- fraud and playlist concentration risk.

## Scenario engine

Generate at least downside, base, and upside scenarios. Each scenario stores assumptions, forecast cash flows, discount-rate components, terminal treatment, and sensitivity tables.

## Analytics versus valuation

Operational analytics may update daily. Formal valuation snapshots should update on a controlled cadence or material event, not continuously with every play. Show the distinction in the UI.

## No price oracle

The valuation service is not an on-chain price oracle and must not automatically set token sale or secondary-market prices.
