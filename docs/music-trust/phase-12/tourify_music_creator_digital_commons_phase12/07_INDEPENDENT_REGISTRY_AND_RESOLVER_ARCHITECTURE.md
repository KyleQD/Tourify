# Independent Registry and Resolver Architecture

## Purpose

Design independently hosted registries and resolvers for participants, issuers, schemas, services and minimal rights references.
...
- `resolver_snapshots`
- `registry_reconciliation_events`

## Primary workflows

1. Submit registry entry.
2. Validate authority and policy.
3. Publish minimal projection.
4. Suspend, revoke, correct or supersede.
...
- Registry does not create rights.
- Multiple operators can serve consistent views.
- Signed snapshots and transparency logs.
- Dispute and freshness status always visible.
...
- Registry writes are controlled by source-specific workflows; resolver reads are public only through approved projections.
- A verifier must be able to validate signatures and source freshness without contacting Tourify.
