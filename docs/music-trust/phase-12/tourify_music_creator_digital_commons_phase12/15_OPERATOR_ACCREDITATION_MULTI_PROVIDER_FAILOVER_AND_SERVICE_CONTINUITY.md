# Operator Accreditation, Multi-Provider Failover and Service Continuity

## Purpose

Require independently replaceable operators, synchronized projections, failover and continuity drills.
...
- `replication_status`
- `failover_drills`
- `service_continuity_events`

## Primary workflows

1. Accredit scoped operator.
2. Synchronize signed projections.
3. Monitor service levels.
4. Fail over or replace operator.
5. Reconcile after recovery.
...
- No operator is authoritative by availability alone.
- State reconciliation is deterministic.
- Split-brain handling documented.
- Operator removal preserves participant portability.
...
- Operators may host registries, resolvers, directories, status services or conformance systems under separate scopes.
- Critical writes require consensus or steward authorization; reads may be served by independently verifiable replicas.
