...
Define route contracts, idempotency, outboxes, worker boundaries, provider reconciliation and replay protection.
...
2. Authorize exact scope.
3. Write state and outbox atomically.
4. Deliver idempotently.
5. Reconcile provider response.
...
- No server actions for public API contracts.
- Signed webhooks and replay windows.
- Dead-letter and compensating queues.
...
- Routes live under `app/api/creator-digital-commons/**`; long-running work goes to workers.
- External records are versioned and never silently overwritten.
