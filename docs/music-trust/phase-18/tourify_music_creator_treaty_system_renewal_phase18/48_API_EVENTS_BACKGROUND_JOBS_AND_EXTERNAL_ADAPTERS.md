...
Defines routes, Zod schemas, authorization, outbox workers, signatures, replay protection, reconciliation and dead letters.
...
External calls use signed envelopes where supported, deterministic idempotency keys, replay windows, inbox/outbox state, retries, reconciliation, dead-letter queues and compensating actions. Webhooks verify signatures before parsing durable state and never trust provider status without reconciliation.
