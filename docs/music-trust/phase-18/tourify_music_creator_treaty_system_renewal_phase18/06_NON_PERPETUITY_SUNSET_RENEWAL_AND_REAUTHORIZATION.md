# Non-perpetuity, sunset, renewal and reauthorization

## Purpose

Requires affirmative renewal and distinguishes automatic expiry, review deadlines, renewal, revival and replacement.
...
Every renewable authority has `review_due_at`, `sunset_at`, `renewed_from_id`, `renewal_decision_id`, `grace_policy`, `continuity_scope` and `post_expiry_behavior`. Default post-expiry behavior is read-only historical access plus essential continuity; new high-impact actions are denied.
