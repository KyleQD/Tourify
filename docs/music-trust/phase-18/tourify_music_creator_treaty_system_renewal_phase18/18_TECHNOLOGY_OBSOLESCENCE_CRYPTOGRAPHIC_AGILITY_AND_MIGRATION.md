# Technology obsolescence, cryptographic agility and migration

## Purpose

Plans format, dependency, signature, key, algorithm, hardware and platform migration without rewriting historical meaning.
...
Maintain an algorithm and dependency inventory, supported-verification horizon, migration deadline, dual-signature transition window, downgrade resistance, test vectors and archival verification path. New cryptography must not invalidate historical evidence; old verification environments may need isolated, reproducible preservation.
