...
Define route, event, worker and partner-integration contracts.
...
1. Use `app/api/creator-protocol-constitution/**` with colocated Zod.
2. Use idempotency, signed webhooks and outbox delivery.
3. Separate public projections from privileged actions.

## Durable records

- `constitutional_api_requests`
- `outbox_events`
- `webhook_receipts`
- `reconciliation_runs`
...
- Use `requireApiUser` and `jsonError`.
- Replay-safe external writes.
- No cached credential authorizes high-impact action.
