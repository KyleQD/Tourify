# Multi-Root Trust Registries and Recognition Compact

## Purpose

Avoid a single constitutional root of trust while supporting predictable recognition.
...
1. Define root eligibility, recognition, suspension and removal.
2. Support jurisdictional and community roots with conflict metadata.
3. Require verifiers to check current source status.

## Durable records

- `constitutional_trust_roots`
- `recognition_agreements`
- `root_status_events`
- `trust_conflicts`
...
- No root may assert universal authority.
- Conflicting roots are disclosed, not silently merged.
- Cached status cannot authorize high-impact action.
