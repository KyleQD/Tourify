# Resilience, redundancy, failover, network partition and reconciliation

## Purpose

Keep trusts independently operational during outages while preventing split-brain authority and stale decisions.

## Required design decisions

- Define local autonomy and fail-closed high-impact actions.
- Use signed snapshots with expiry.
- Reconcile divergent events deterministically.
- Publish stale or partitioned status without leaking restricted data.
...
1. Define local autonomy and fail-closed high-impact actions.
2. Use signed snapshots with expiry.
3. Reconcile divergent events deterministically.
4. Publish stale or partitioned status without leaking restricted data.
