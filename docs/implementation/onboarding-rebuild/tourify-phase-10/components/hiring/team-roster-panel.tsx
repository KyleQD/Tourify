...
export function TeamRosterPanel({ employer }: TeamRosterPanelProps) {
  ...
  const queryParams = useMemo(() => {
    ...
    return params
  }, [search, status, complianceStatus, department])
    ...
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Active staff, Work Mode access, compliance status, and shift/zone assignments.
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          ...
          <CardContent className="text-2xl font-bold">{result?.total ?? 0}</CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Active</CardTitle>
          </CardHeader>
          <CardContent className="text-2xl font-bold">{result?.statusCounts.active ?? 0}</CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Needs review</CardTitle>
          </CardHeader>
          <CardContent className="text-2xl font-bold">{result?.complianceCounts.needs_review ?? 0}</CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Compliant</CardTitle>
          </CardHeader>
          <CardContent className="text-2xl font-bold">{result?.complianceCounts.compliant ?? 0}</CardContent>
        </Card>
      </div>
