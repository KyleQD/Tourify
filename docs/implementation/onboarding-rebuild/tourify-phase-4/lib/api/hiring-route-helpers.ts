...
/**
 * Auth adapter for Phase 4 route handlers.
 *
 * This works with `Authorization: Bearer <supabase_access_token>` immediately.
 * In the Tourify repo, Cursor should merge this with the existing cookie/session
 * Supabase helper if admin dashboard requests rely on same-origin cookies.
 */
export async function getAuthenticatedUserId({
  ...
  if (!bearerToken) {
    return fail({
      code: "UNAUTHORIZED",
      message:
        "Authentication token was not found. Merge getAuthenticatedUserId() with the repo's existing cookie/session Supabase helper if dashboard requests use cookies.",
    })
  }
  ...
  if (error || !data.user?.id) {
    return fail({ code: "UNAUTHORIZED", message: "User session is invalid or expired.", details: error })
  }
...
export function getListFiltersFromRequest(request: NextRequest): {
  ...
  return {
    ...
    jobPostingId: params.get("job_posting_id") ?? params.get("jobPostingId") ?? undefined,
