...
const ENTITY_LOOKUP_CONFIG: Record<HiringEntityType, EntityLookupConfig> = {
  venue: {
    tableName: "venues",
    displayColumns: ["name", "title", "display_name"],
  },
  organization: {
  ...
  artist: {
    tableName: "artists",
    displayColumns: ["name", "artist_name", "display_name"],
  },
}
...
function normalizeHiringEntityInput(args: ResolveHiringEntityArgs): HiringServiceResult<{
  ...
  if (args.entityType && args.entityId) {
    return ok({ entityType: args.entityType, entityId: args.entityId })
  }

  if (args.venueId) {
    return ok({ entityType: "venue", entityId: args.venueId })
  }
 ...
 *
 * This is the bridge between legacy venue-only code and the new universal model.
 * During migration, callers may pass `venueId`; new callers should pass `entityType` + `entityId`.
 */
