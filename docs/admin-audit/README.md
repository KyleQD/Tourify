...
- The post-implementation UX audit traceability program lives in `registry/ux-audit-program.json`. It maps every `AUX-*` finding to one `ADMUX-*` implementation task and the existing canonical batches without introducing a second status field.
...
- `npm run import:admin-ux-audit` reimports the attached audit CSVs, verifies the 158 → 54 → 18 crosswalk, and refreshes the status-free canonical mapping.
...
The Phase 00 worktree assessment is recorded in [ux/PHASE_00_RECONCILIATION.md](ux/PHASE_00_RECONCILIATION.md). It classifies partial UX work for reuse without granting completion credit.
