...
function hasApprovedException(sourceSql, manifest, type) {
  ...
  return manifest.exceptions?.some(
    (exception) => exception.type === type && exception.sourceSha256 === sourceSha256,
  ) ?? false
}

function unsafeUpdateStatements(sql, sourceSql, manifest) {
  if (hasApprovedException(sourceSql, manifest, "unscoped-update-reviewed")) return []
  return sql
    .split(";")
...
function scanFile(file, sql, manifest = null, options = {}) {
  ...
  if (unsafeUpdateStatements(topLevelSql, sql, manifest).length > 0)
    failures.push("UPDATE backfill without a WHERE clause")

  if (unsafeInsertSelectStatements(topLevelSql, sql, manifest).length > 0)
    failures.push("INSERT ... SELECT backfill without a WHERE clause or scoped review marker")

  if (!options.immutableHistory && unsafeConstraintStatements(topLevelSql, sql, manifest).length > 0)
    failures.push("existing-table FK/CHECK constraint missing NOT VALID or reviewed lock-budget marker")

  if (!options.immutableHistory && unsafeNotNullStatements(topLevelSql, sql, manifest).length > 0)
...
export function validateManifest(manifest, file, options = {}) {
  ...
  for (const exception of manifest?.exceptions ?? []) {
    ...
    if (!["scoped-insert-select", "unscoped-update-reviewed", "blocking-constraint-reviewed", "not-null-reviewed"].includes(exception?.type))
    ...
    if (exception?.sourceSha256 !== undefined && !/^[0-9a-f]{64}$/.test(exception.sourceSha256))
      failures.push(`exception ${exception?.id ?? "<unknown>"}.sourceSha256 must be a SHA-256`)
  }
...
export function main() {
  ...
  const args = cliArgs.filter((a) => a.endsWith(".sql"))
