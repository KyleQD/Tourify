...
async function checkEmployerDashboard(args: {
  ...
  const result = await fetchJson({ url: `${args.appUrl}/api/hiring/dashboard?${query}`, actorToken: args.scenario.actorToken })
...
async function checkApplicationsApi(args: {
  ...
  const query = buildEmployerQueryParams(args.scenario)
  const result = await fetchJson({ url: `${args.appUrl}/api/hiring/applications?${query}`, actorToken: args.scenario.actorToken })
...
async function checkRosterApi(args: {
  ...
  const query = buildEmployerQueryParams(args.scenario)
  const result = await fetchJson({ url: `${args.appUrl}/api/hiring/roster?${query}`, actorToken: args.scenario.actorToken })
...
async function checkTokenOnboarding(args: {
  ...
  const result = await fetchJson({ url: `${args.appUrl}/api/onboarding/${args.scenario.invitationToken}`, actorToken: args.scenario.actorToken })
...
async function checkOnboardingResponsesColumn(args: {
  supabase: SupabaseClient
}): Promise<Phase13CheckResult> {
  ...
  if (error) {
    return fail({
      scenarioKey: "global",
      name: "Column exists: staff_onboarding_candidates.onboarding_responses",
      message: error.message,
    })
  }
  ...
  return pass({
    scenarioKey: "global",
    name: "Column exists: staff_onboarding_candidates.onboarding_responses",
    message: "Onboarding responses are stored as JSONB on staff_onboarding_candidates.",
  })
}
...
async function run() {
  ...
  results.push(await checkOnboardingResponsesColumn({ supabase }))
