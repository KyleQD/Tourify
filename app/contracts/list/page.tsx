...
export default function ContractsListPage() {
  ...
  useEffect(() => {
    ...
    if (!user) {
      router.push(`/login?redirectTo=${encodeURIComponent("/contracts/list")}`)
      return
    }
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        >
          <div>
            ...
            <p className={dashboardCreatePattern.subtleText}>
              Agreements you sent or were invited to sign.
            </p>
          </div>
          <Button asChild variant="secondary" className={dashboardCreatePattern.btnOutline}>
            <Link href="/artist/business/contracts">Create new contract (artists)</Link>
          </Button>
        </div>
        ...
        {rows.length === 0 ? (
          <Card className={cn(dashboardCreatePattern.panel, "border-slate-700/50")}>
            <CardContent className="py-12 text-center text-slate-400 space-y-4">
              ...
              <Button asChild variant="secondary" size="sm" className="mt-2">
                <Link href="/artist/business/contracts">Create a contract (artists)</Link>
              </Button>
            </CardContent>
          </Card>
        ) : (
          <div className="space-y-3">
            {rows.map((r) => {
              const role = r.user_id === user.id ? "You are the sender" : "You were invited to sign"
              return (
