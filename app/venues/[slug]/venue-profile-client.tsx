...
interface VenueProfileClientProps {
  slug: string
  /** VEN-019: server-rendered public contract payload; skips the client fetch. */
  initialVenue?: VenueProfile | null
}

export function VenueProfileClient({ slug, initialVenue }: VenueProfileClientProps) {
  const router = useRouter()
  const [venue, setVenue] = useState<VenueProfile | null>(initialVenue ?? null)
  ...
  useEffect(() => {
    // VEN-019: primary content is server-rendered; only hydrate when the
    // shell could not provide it (e.g. fallback paths).
    if (slug && !initialVenue) {
