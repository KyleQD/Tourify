...
export default function PressEditor({ postId, onBack, initialFormat }: PressEditorProps) {
  const { user } = useArtist()
  const { actingHeaders, isActingReady } = useActingContext()
  const [post, setPost] = useState<PressPost>({
    title: '',
    slug: '',
    content: '',
    excerpt: '',
  ...
  useEffect(() => {
    ...
    loadPost()
  }, [postId, user, isActingReady, initialFormat])
  ...
  async function loadPost() {
    ...
    try {
      ...
      setPost({
        ...
        categories,
        format: parsePressFormat(article.format, 'blog'),
        subtitle: article.subtitle || '',
        boilerplate: article.boilerplate || '',
        embargo_until: toDatetimeLocalValue(article.embargo_until),
  ...
  function showPublishSuccessToast(article: { id?: string; slug?: string }) {
    const id = article.id || post.id || ''
    const slug = article.slug || post.slug || ''

    if (post.format === 'press_release') {
      toast.success('Press release saved', {
        action: {
          label: 'View release',
          onClick: () => window.open(`/artist/press/releases/${id}`, '_blank', 'noopener,noreferrer'),
        },
      })
      return
    }
    ...
    if (post.format === 'article') {
      toast.success('Published — it will appear in News → Articles', {
        action: slug
          ? {
              label: 'View live',
  ...
  async function handleSave(status: 'draft' | 'published' = 'draft') {
    ...
    try {
      ...
      const payload = {
        ...
        format: post.format,
        subtitle: post.format === 'article' ? (post.subtitle?.trim() || undefined) : undefined,
        boilerplate: post.format === 'press_release' ? (post.boilerplate?.trim() || undefined) : undefined,
        embargoUntil: post.format === 'press_release'
          ? fromDatetimeLocalValue(post.embargo_until || '')
          : undefined,
      }
      ...
      if (status === 'published' && (slug || post.format === 'press_release')) {
        showPublishSuccessToast(data.article || {})
      } else {
        toast.success(
          postId ? 'Press item updated successfully!' : 'Draft saved successfully!'
        )
      }
  ...
  const formatLabel = PRESS_FORMAT_LABELS[post.format]
  const publicUrl = post.id && post.slug
    ? publicUrlForPressItem({ format: post.format, slug: post.slug, id: post.id })
    : null
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <h1 className="text-2xl font-bold text-white">
              {postId ? `Edit ${formatLabel}` : `Create ${formatLabel}`}
            </h1>
            <PressTypeBadge format={post.format} />
          </div>
        </div>
          ...
          {isPreview ? (
            ...
            <Card className="bg-slate-900/50 border-slate-700/50">
              ...
              <CardContent className="space-y-4">
                ...
                {postId && post.slug && (
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-gray-500">
                      Public URL: {publicUrl || `/blog/${post.slug}`}
                    </p>
                  </div>
                )}
                ...
                {post.format === 'press_release' && (
                  <>
                    <div className="space-y-2">
                      <Label htmlFor="boilerplate" className="text-gray-300">Boilerplate</Label>
                      <Textarea
                        id="boilerplate"
                        value={post.boilerplate || ''}
                        onChange={(e) => setPost(prev => ({ ...prev, boilerplate: e.target.value }))}
                        placeholder="About the company / artist boilerplate..."
                        className="bg-slate-800 border-slate-700 text-white min-h-[100px]"
                      />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="embargo_until" className="text-gray-300">Embargo until</Label>
                      <Input
                        id="embargo_until"
                        type="datetime-local"
                        value={post.embargo_until || ''}
                        onChange={(e) => setPost(prev => ({ ...prev, embargo_until: e.target.value }))}
                        className="bg-slate-800 border-slate-700 text-white"
                      />
                    </div>
                  </>
...
function PreviewPane({
  ...
  return (
    <div className="space-y-6 rounded-xl border border-slate-700 bg-slate-900/50 p-6">
      ...
      {format === 'press_release' && boilerplate && (
        <div className="border-t border-white/10 pt-4">
          <p className="text-xs font-medium uppercase tracking-wide text-slate-500 mb-2">Boilerplate</p>
          <p className="text-sm leading-relaxed text-slate-400 whitespace-pre-wrap">{boilerplate}</p>
        </div>
      )}
