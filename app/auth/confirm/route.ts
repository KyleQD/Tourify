...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const safeNext = normalizePostLoginRedirect(nextParam)

    if (type === "signup" || type === "invite" || type === "email" || type === "email_change") {
      if (data.session?.user) {
        return NextResponse.redirect(
          `${publicOrigin}${safeNext === "/dashboard" ? "/dashboard?welcome=true" : safeNext}`,
        )
      }
      ...
      return NextResponse.redirect(
        `${publicOrigin}/login?message=email_confirmed${emailQuery}&redirectTo=${encodeURIComponent(safeNext)}`
      )
    }
