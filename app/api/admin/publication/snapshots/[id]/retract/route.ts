...
/** PUB-207 — Retract a committed publication; invalidate access; notify recipients. */
export const POST = withAdminCapability(
  ...
  async (request: NextRequest, { supabase, user, admin }) => {
    ...
    try {
      const result = await retractPublicationSnapshot({
        supabase,
        orgId: admin.orgId,
        actorUserId: user.id,
        snapshotId,
      ...
      const message = error instanceof Error ? error.message : "Retraction failed"
