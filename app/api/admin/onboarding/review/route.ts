...
export async function POST(request: NextRequest) {
  return withAdminAuth(async (req) => {
    ...
    if (!candidateId || (action !== 'approve' && action !== 'reject')) {
      return NextResponse.json({ success: false, error: 'candidate_id and valid action are required' }, { status: 400 })
    }
    ...
    const updated = await AdminOnboardingStaffService.updateOnboardingProgress(candidateId, {
      progress: action === 'approve' ? 100 : Number(candidate?.onboarding_progress || 0),
      status: action === 'approve' ? 'completed' : 'rejected',
      stage: action === 'approve' ? 'approved' : 'rejected',
      notes: body.review_notes || undefined,
    })
