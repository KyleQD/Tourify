...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  return withAdminCapability('logistics.view', async (_request, { supabase, user, admin }) => {
    try {
      const access = requireSiteMapAccess(await getSiteMapAccess(supabase as any, id, user.id, { requiredOrgId: admin.orgId }), 'read')
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  return withAdminCapability('logistics.manage', async (_request, { supabase, user, admin }) => {
    try {
      ...
      const access = requireSiteMapAccess(await getSiteMapAccess(supabase as any, id, user.id, { requiredOrgId: admin.orgId }), 'edit')
      if (!access.ok) return NextResponse.json({ error: access.error }, { status: access.status })
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  return withAdminCapability('logistics.manage', async (_request, { supabase, user, admin }) => {
    try {
      const access = requireSiteMapAccess(await getSiteMapAccess(supabase as any, id, user.id, { requiredOrgId: admin.orgId }), 'manage')
      if (!access.ok) return NextResponse.json({ error: access.error }, { status: access.status })
