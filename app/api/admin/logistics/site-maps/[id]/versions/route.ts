...
export const GET = withAdminCapability(
  ...
  async (request: NextRequest, { supabase, user }) => {
    ...
    const { data, error } = await supabase
      .from("map_versions")
      .select(
        "id, site_map_id, version_name, description, version_number, is_current, change_summary, published_by, published_at, status, created_at, created_by",
      )
    ...
    if (error) {
      if (error.code === "42P01")
        return NextResponse.json({ versions: [], needsMigration: true });
