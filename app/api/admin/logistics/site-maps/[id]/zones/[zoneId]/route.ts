...
import { syncZoneOwnershipToRoster } from "@/lib/site-map/zone-roster-sync";
  ...
  return withAdminCapability(
    ...
    async (req, { supabase, user, admin }) => {
      ...
      try {
        ...
        // Keep roster / staff_shifts zone linkage aligned with map zone
        // ownership via the shared event_zones bridge.
        try {
          const eventId = access.siteMap?.event_v2_id;
          if (eventId && (zone.event_zone_id || zone.lead_user_id)) {
            await syncZoneOwnershipToRoster({
              supabase,
              siteMapZoneId: zone.id,
              eventId,
              leadUserId: zone.lead_user_id,
              assignedDepartment: zone.assigned_department,
              actorUserId: user.id,
            });
          }
        } catch (syncError) {
          console.warn("[Site-map zone] Roster sync failed (non-fatal)", {
            correlationId: admin.correlationId,
            error: syncError,
          });
        }
