...
import { validateLogisticsPlan } from "@/lib/logistics/plans"
...
export const POST = withAdminCapability("logistics.manage", async (request: NextRequest, { supabase, user, admin }) => {
  ...
  try {
    ...
    const plan = await validateLogisticsPlan({
      supabase,
      userId: user.id,
      orgId: admin.orgId,
      tourId,
    ...
    const code = error && typeof error === "object" && "code" in error ? String((error as { code?: unknown }).code) : "validation_failed"
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to validate logistics plan.", code }, { status })
  }
})
