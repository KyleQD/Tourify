...
import { getTourTransition, TOUR_TRANSITION_COMMANDS } from "@/lib/admin/tour-lifecycle"
 ...
 */
export const POST = withAdminCapability((request) => {
  const command = extractCommand(request.url)
  return isTourTransitionCommand(command)
    ? getTourTransition(command)?.capability || "tour.manage"
    : "tour.manage"
