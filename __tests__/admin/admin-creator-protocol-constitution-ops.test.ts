...
const mocks = vi.hoisted(() => {
  ...
  const from = vi.fn((table: string) => {
    if (table === 'creator_protocol_outbox') return { select: outboxSelect }
    if (table === 'feature_flags') return { select: flagSelect, update }
    if (table === 'creator_protocol_audit_events') return { insert: auditInsert }
...
vi.mock('@/lib/music/creator-protocol-constitution/creator-protocol-constitution-flags', () => ({
  resolveCreatorProtocolConstitutionFlags: mocks.resolveFlags,
}))
...
import { GET, POST } from '@/app/api/admin/creator-protocol-constitution/ops/route'

function opsRequest(body?: Record<string, unknown>) {
  return new NextRequest('https://tourify.test/api/admin/creator-protocol-constitution/ops', body ? {
    method: 'POST',
...
describe('Creator-protocol-constitution platform operations', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('preserves the disabled-readiness response without requesting a trusted client', async () => {
    mocks.resolveFlags.mockResolvedValueOnce({ creator_protocol_constitution_readiness_enabled: false })

    const response = await POST(opsRequest({ action_type: 'fundamental_provision_freeze' }))
  ...
  it('preserves the enabled pending-outbox and protocol flag read model', async () => {
    mocks.resolveFlags.mockResolvedValueOnce({ creator_protocol_constitution_readiness_enabled: true })
    mocks.outboxLimit.mockResolvedValueOnce({
      data: [{ id: 'outbox-1', event_type: 'constitution.changed', status: 'pending' }],
    })
    mocks.flagLike.mockResolvedValueOnce({
      data: [{ key: 'creator_protocol_fundamental_provisions_enabled', enabled: true }],
    })
    ...
    expect(mocks.flagLike).toHaveBeenCalledWith('key', 'creator_protocol_%')
    await expect(response.json()).resolves.toEqual({
      data: {
        pendingOutbox: [{ id: 'outbox-1', event_type: 'constitution.changed', status: 'pending' }],
        flags: [{ key: 'creator_protocol_fundamental_provisions_enabled', enabled: true }],
      },
      enabled: true,
    })
  })

  it('preserves fundamental-provision freeze keys, schema defaults, and audit fields', async () => {
    mocks.resolveFlags.mockResolvedValueOnce({ creator_protocol_constitution_readiness_enabled: true })
    const expectedKeys = [
    ...
    expect(mocks.auditInsert).toHaveBeenCalledWith({
      event_type: 'fundamental_provision_freeze',
      actor_type: 'admin',
      ...
      event_hash: expect.stringMatching(
        /^cpc-ops:fundamental_provision_freeze:00000000-0000-4000-8000-000000000001:\d+$/,
      ),
    })
    await expect(response.json()).resolves.toEqual({
