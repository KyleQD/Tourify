...
const mocks = vi.hoisted(() => {
  ...
  const from = vi.fn((table: string) => {
    if (table === 'creator_interop_institution_outbox') return { select: outboxSelect }
    if (table === 'feature_flags') return { select: flagSelect, update }
    if (table === 'creator_interop_institution_audit_events') return { insert: auditInsert }
...
vi.mock('@/lib/music/creator-interoperability-institution/creator-interop-institution-flags', () => ({
  resolveCreatorInteropInstitutionFlags: mocks.resolveFlags,
}))
...
import { GET, POST } from '@/app/api/admin/creator-interoperability-institution/ops/route'

function opsRequest(body?: Record<string, unknown>) {
  return new NextRequest('https://tourify.test/api/admin/creator-interoperability-institution/ops', body ? {
    method: 'POST',
...
describe('Creator-interoperability-institution platform operations', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('preserves the disabled-readiness response without requesting a trusted client', async () => {
    mocks.resolveFlags.mockResolvedValueOnce({ creator_interop_institution_readiness_enabled: false })

    const response = await POST(opsRequest({ action_type: 'institution_freeze' }))
  ...
  it('preserves the enabled pending-outbox and institution flag read model', async () => {
    mocks.resolveFlags.mockResolvedValueOnce({ creator_interop_institution_readiness_enabled: true })
    mocks.outboxLimit.mockResolvedValueOnce({
      data: [{ id: 'outbox-1', event_type: 'institution.changed', status: 'pending' }],
    })
    mocks.flagLike.mockResolvedValueOnce({
      data: [{ key: 'creator_interop_institution_public_registry_enabled', enabled: true }],
    })
    ...
    expect(mocks.flagLike).toHaveBeenCalledWith('key', 'creator_interop_institution_%')
    await expect(response.json()).resolves.toEqual({
      data: {
        pendingOutbox: [{ id: 'outbox-1', event_type: 'institution.changed', status: 'pending' }],
        flags: [{ key: 'creator_interop_institution_public_registry_enabled', enabled: true }],
      },
      enabled: true,
    })
  })

  it('preserves institution-freeze keys, schema defaults, and actor-attributed audit fields', async () => {
    mocks.resolveFlags.mockResolvedValueOnce({ creator_interop_institution_readiness_enabled: true })
    const expectedKeys = [
    ...
    expect(mocks.auditInsert).toHaveBeenCalledWith({
      ...
      event_hash: expect.stringMatching(
        /^p16-ops:institution_freeze:00000000-0000-4000-8000-000000000001:\d+$/,
      ),
    })
    await expect(response.json()).resolves.toEqual({
