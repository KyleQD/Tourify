...
-- Approval moves applicants into onboarding where documents and agreements are
-- collected. The previous trigger blocked every approval unless the applicant
-- already had verified documents and signed agreements on file, which breaks
-- Quick Apply and manual admin review flows.
--
-- Keep only job-specific required_certifications enforcement; generic
-- pre-approval document/agreement checks are handled in application code via
-- FEATURE_HIRING_ELIGIBILITY_GATE (off/shadow/enforce).
