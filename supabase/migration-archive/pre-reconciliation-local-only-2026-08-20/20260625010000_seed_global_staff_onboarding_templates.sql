...
-- Legacy tables may use required_fields instead of fields, or omit employment_type entirely.
...
begin
  if exists (
    ...
    where table_schema = 'public'
      and table_name = 'staff_onboarding_templates'
    ...
    execute $sql$
      ...
      where (fields is null or fields = '[]'::jsonb)
        and required_fields is not null
        and required_fields <> '[]'::jsonb
    $sql$;
  end if;
end $$;

-- Global templates use employer_entity_* scope with null venue_id.
-- Legacy databases often enforced venue_id NOT NULL; relax it for universal hiring.
alter table public.staff_onboarding_templates
