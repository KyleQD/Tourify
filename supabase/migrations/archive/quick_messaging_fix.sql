...
-- Ensure sender_id foreign key exists (this is the key fix for the error)
ALTER TABLE messages ADD CONSTRAINT IF NOT EXISTS messages_sender_id_fkey 
...
CREATE POLICY IF NOT EXISTS "Users can view their messages" ON messages
    FOR SELECT USING (
        EXISTS (
