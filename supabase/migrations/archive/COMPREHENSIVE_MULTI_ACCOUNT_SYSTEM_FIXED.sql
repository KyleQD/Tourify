-- =====================================================
-- COMPREHENSIVE MULTI-ACCOUNT SYSTEM (FIXED VERSION)
-- =====================================================
...
-- This version properly handles existing functions and potential conflicts
...
-- =====================================================
-- STEP 3: Drop and Recreate Account Management Functions
-- =====================================================
...
BEGIN
  ...
  IF p_profile_table = 'profiles' THEN
    SELECT COALESCE(full_name, 'User'), username, avatar_url, is_verified
...
BEGIN
  FOR user_record IN (
    ...
    RAISE NOTICE 'Created primary account for user: % (ID: %)', COALESCE(user_record.full_name, 'User'), account_id;
  END LOOP;
END $$;
...
DROP POLICY IF EXISTS "Users can insert posts through their accounts" ON posts;
DROP POLICY IF EXISTS "Users can update posts through their accounts" ON posts;
DROP POLICY IF EXISTS "Users can delete posts through their accounts" ON posts;
