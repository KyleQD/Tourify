...
begin
  ...
  if not exists (
    ...
    where tc.constraint_type = 'FOREIGN KEY' and kcu.table_schema = 'public'
      and kcu.table_name = 'staff_onboarding_templates' and kcu.column_name = 'parent_template_id'
  )
  then
    alter table public.staff_onboarding_templates
...
create index if not exists idx_staff_onboarding_templates_parent on public.staff_onboarding_templates(parent_template_id);;
