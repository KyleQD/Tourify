-- Active logistics schema reconciliation.
...
-- Reviewed logistics foundation tables that were previously archive-only.
...
-- Keep the active chain service-role-only until scoped client policies exist.
do $$
begin
  execute 'drop policy if exists logistics_acks_select_own_or_admin on public.logistics_acknowledgements';
  execute 'drop policy if exists logistics_acks_insert_authenticated on public.logistics_acknowledgements';
  execute 'drop policy if exists logistics_acks_update_own on public.logistics_acknowledgements';
  execute 'drop policy if exists equipment_reservations_authenticated_all on public.equipment_reservations';
  execute 'drop policy if exists backline_requirements_authenticated_all on public.backline_requirements';
  execute 'drop policy if exists backline_fulfillments_authenticated_all on public.backline_fulfillments';
  execute 'drop policy if exists backline_subs_authenticated_all on public.backline_substitution_approvals';
  execute 'drop policy if exists catering_services_authenticated_all on public.catering_services';
  execute 'drop policy if exists catering_snapshots_authenticated_all on public.catering_headcount_snapshots';
  execute 'drop policy if exists catering_dietary_authenticated_all on public.catering_dietary_summaries';
  execute 'drop policy if exists comms_plans_authenticated_all on public.logistics_comms_plans';
  execute 'drop policy if exists comms_channels_authenticated_all on public.logistics_comms_channels';
end $$;
