...
begin
  ...
  -- Canonical Tourify entity RBAC (has_entity_permission from entity_rbac_core migration).
  if to_regprocedure('public.has_entity_permission(uuid,text,uuid,text)') is not null then
    if v_entity_type = 'venue' and (
      public.has_entity_permission(p_user_id, 'Venue', p_entity_id, 'ASSIGN_EVENT_ROLES')
      or public.has_entity_permission(p_user_id, 'Venue', p_entity_id, 'MANAGE_MEMBERS')
    ) then
      return true;
    end if;

    if v_entity_type = 'organization' and (
      public.has_entity_permission(p_user_id, 'Organizer', p_entity_id, 'ASSIGN_EVENT_ROLES')
      or public.has_entity_permission(p_user_id, 'Organizer', p_entity_id, 'MANAGE_MEMBERS')
    ) then
      return true;
    end if;

    if v_entity_type = 'artist' and (
      public.has_entity_permission(p_user_id, 'Artist', p_entity_id, 'ASSIGN_EVENT_ROLES')
      or public.has_entity_permission(p_user_id, 'Artist', p_entity_id, 'MANAGE_MEMBERS')
    ) then
      return true;
    end if;
  end if;

  -- Profile ownership fallbacks used by acting-context.ts
  if v_entity_type = 'venue'
     and to_regclass('public.venue_profiles') is not null
     and public._tourify_has_columns('venue_profiles', array['id', 'user_id']) then
    execute 'select exists (select 1 from public.venue_profiles where id = $1 and user_id = $2)'
      into v_has_access
  ...
  if v_entity_type = 'organization'
     and to_regclass('public.organizer_accounts') is not null
     and public._tourify_has_columns('organizer_accounts', array['id', 'user_id']) then
  ...
  if v_entity_type = 'artist'
     and to_regclass('public.artist_profiles') is not null
     and public._tourify_has_columns('artist_profiles', array['id', 'user_id']) then
    execute 'select exists (select 1 from public.artist_profiles where id = $1 and user_id = $2)'
      into v_has_access
      using p_entity_id, p_user_id;
    if v_has_access then
      return true;
