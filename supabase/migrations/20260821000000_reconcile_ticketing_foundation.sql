-- Reconciled additive migration for the active ticketing foundation.
-- Reviewed from the archived local-only source; this is a new migration file, not a history restore.
-- It must precede 20260821025543_unified_guest_list_admissions.sql because that
-- migration extends the ticketing foundation tables created here.
...
do $$ begin
  ...
  alter table ticket_types
    ...
    check (visibility in ('public', 'private', 'hidden', 'access_code')) not valid;
...
alter table tickets
  ...
  foreign key (allocation_id) references ticket_allocations(id) on delete set null not valid;
