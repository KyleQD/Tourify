...
export function CandidateDocumentReview({ documents = [], disabled, onReviewDocument }: CandidateDocumentReviewProps) {
  ...
  if (documents.length === 0) {
    return (
      <WorkforcePanel className="space-y-2 p-5">
        <h3 className="text-base font-medium text-white">Documents</h3>
        <p className="text-sm text-slate-400">No onboarding documents have been submitted yet.</p>
      </WorkforcePanel>
    )
  }
  ...
  return (
    <WorkforcePanel className="space-y-4 p-5">
      <h3 className="text-base font-medium text-white">Document Review</h3>
      <div className="space-y-4">
        {documents.map((document) => (
          <div key={document.id} className="rounded-xl border border-slate-700/60 bg-slate-900/40 p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              ...
              <div className="flex shrink-0 flex-wrap items-center gap-2">
                ...
                {document.signedUrl ? (
                  <Button size="sm" variant="outline" asChild className="border-slate-700/60 bg-slate-900/50 hover:border-cyan-400/40">
