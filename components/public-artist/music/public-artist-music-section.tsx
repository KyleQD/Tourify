...
export function PublicArtistMusicSection({
  ...
  const addToLibrary = async (track: PublicArtistTrackDTO) => {
    if (!viewer.isAuthenticated) {
      toast.error("Sign in to add tracks to your library")
      return
    }
    ...
    if (!res.ok) {
      ...
      toast.error(body?.error?.message || body?.message || "Could not add track")
      return
    }
    toast.success("Added to your library")
  }

  const featureOnProfile = async (track: PublicArtistTrackDTO) => {
    if (!viewer.isAuthenticated) {
      toast.error("Sign in to feature tracks on your profile")
      return
    }
    const res = await fetch("/api/music/profile-featured-track", {
      method: "PATCH",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ musicId: track.id }),
    ...
    if (!res.ok) {
      const body = await res.json().catch(() => ({}))
      toast.error(body?.error?.message || body?.message || "Could not feature track")
      return
    }
        ...
        <div className="flex flex-col gap-4">
          {featuredTrack ? (
            <div className={`${paInset} flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between`}>
              ...
              <div className="flex items-center gap-2">
                {featuredTrack.accessMode === "paid" && featuredTrack.listingId ? (
                  <Button
                    variant="secondary"
                ...
                ) : featuredTrack.allowLibraryAdd && !viewer.isOwner ? (
                  <Button
                    variant="ghost"
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {!viewer.isOwner && t.accessMode === "paid" && t.listingId ? (
                      <Button
                        variant="ghost"
                        size="sm"
                        className="rounded-full text-emerald-300 hover:text-emerald-200"
                        onClick={(e) => {
                          e.preventDefault()
                          e.stopPropagation()
                          buyTrack(t)
                    ...
                    {!viewer.isOwner && t.accessMode === "free" && t.allowLibraryAdd ? (
                      <Button
                        variant="ghost"
                        size="sm"
                    ...
                    {!viewer.isOwner && t.allowProfileFeature ? (
                      <Button
                        variant="ghost"
                        size="sm"
                        className="rounded-full text-white/50 hover:text-white"
