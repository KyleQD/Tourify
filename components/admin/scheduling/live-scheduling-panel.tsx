...
import { WorkforceEmptyState } from "@/components/hiring/workforce-ui"
import { getEmployerQueryString, resolveSchedulingVenueId } from "@/lib/hiring/hiring-dashboard-utils"
...
function normalizeZone(row: Record<string, unknown>): Zone {
  return {
    id: String(row.id ?? ""),
    name: String(row.zone_name ?? row.name ?? "Zone"),
  }
}
...
export function StaffSchedulingTab({ employer }: StaffSchedulingTabProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (zonesRes.status === "fulfilled" && zonesRes.value.ok) {
        ...
        const rows = (d.data || d.zones || []) as Record<string, unknown>[]
        setZones(rows.map(normalizeZone).filter((z) => z.id))
      } else {
        setZones([])
      }

      if (rosterRes.status === "fulfilled" && rosterRes.value.ok) {
        const d = await rosterRes.value.json()
        const payload = d.data
        const rows = Array.isArray(payload)
          ? (payload as Array<{ id: string; name?: string; position?: string }>)
  ...
  async function addShift() {
    if (!venueId) {
      toast.error("A venue context is required to schedule shifts")
      return
    }
    ...
    if (hasConflict) {
      setConflict(
        `${staffMembers.find((s) => s.id === form.staff_member_id)?.full_name || "Staff"} already has a shift that overlaps this time.`,
      )
      return
    }
    ...
    try {
      const res = await fetch("/api/admin/staffing/shifts", {
        ...
        body: JSON.stringify({
          ...
          event_id: eventId,
          ...form,
          zone_assignment: form.zone_assignment || undefined,
        }),
      })
      if (!res.ok) {
  ...
  async function copyWeek() {
    ...
    try {
      ...
      if (failed) throw new Error("One or more shifts failed to copy")
  ...
  if (!venueId) {
    return (
      <WorkforceEmptyState
        icon={MapPin}
        title="Venue context required for scheduling"
        description={
          employer.entityType === "venue"
            ? "Unable to resolve a venue for shift scheduling. Confirm your venue account is active and try again."
            : `Shift scheduling is tied to a venue. Add a venue_id query parameter or assign shifts from an event with a venue for ${employer.displayName}.`
        }
      />
    )
  }
          ...
          <div className="grid grid-cols-8 bg-slate-800/60 border-b border-slate-700/50">
            ...
            {weekDates.map((d, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`p-3 text-xs font-medium text-center ${isToday ? "text-purple-400 bg-purple-500/5" : "text-slate-400"}`}
                >
                    ...
                    <div className="p-3 flex items-center">
                      <div>
                        <p className="text-white text-xs font-medium truncate max-w-[100px]">
                          {member?.full_name ||
                            shifts.find((s) => s.staff_member_id === staffId)?.staff_member_name ||
                            staffId.slice(0, 8)}
                        </p>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(v) => setForm((p) => ({ ...p, zone_assignment: v }))}
                >
                  <SelectTrigger className="bg-slate-800/50 border-slate-700/50 text-white text-sm">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => setForm((p) => ({ ...p, role_assignment: e.target.value }))}
      ...
      <AlertDialog open={!!deleteShift} onOpenChange={() => setDeleteShift(null)}>
        <AlertDialogContent className="bg-slate-900 border-slate-700">
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => deleteShift && removeShift(deleteShift)}
