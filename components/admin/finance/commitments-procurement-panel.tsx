...
interface PurchaseOrder { id: string; poNumber: string | null; vendorName: string | null; status: string; amountMinor: number; currency: string; description: string | null; approvedBy: string | null; createdAt: string }
interface OrderSummary { total: number; draft: number; approved: number; fulfilled: number; cancelled: number; totalAmountMinor: number; currency: string }
interface CommitmentsResponse { orders: PurchaseOrder[]; summary: OrderSummary; unavailable?: boolean; unavailableReason?: string; freshAt: string }
...
/**
 * w15-commitments-procurement
 * FIN-505 / FIN-506 — Commitments, requisitions, POs.
 */
export function CommitmentsProcurementPanel({ tourId, eventId }: { tourId?: string | null; eventId?: string | null }) {
  const { actingAccount } = useActingContext()
  const [state, setState] = useState<"idle" | "loading" | "ready" | "unavailable" | "error">("idle")
  const [orders, setOrders] = useState<PurchaseOrder[]>([])
  const [summary, setSummary] = useState<OrderSummary>({ total: 0, draft: 0, approved: 0, fulfilled: 0, cancelled: 0, totalAmountMinor: 0, currency: "USD" })
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/admin/finances/commitments?${params}`)
      const json = (await res.json()) as CommitmentsResponse & { error?: string }
      ...
      setSummary(json.summary ?? { total: 0, draft: 0, approved: 0, fulfilled: 0, cancelled: 0, totalAmountMinor: 0, currency: "USD" })
  ...
  if (state === "idle" || state === "loading") return <Card className="bg-slate-900/60 border border-slate-700/50 rounded-sm"><CardContent className="p-4"><div className="flex items-center gap-2 text-slate-400 text-sm"><RefreshCw className="h-3.5 w-3.5 animate-spin" />Loading commitments…</div></CardContent></Card>
    ...
    <Card className="bg-slate-900/60 border border-slate-700/50 backdrop-blur-sm rounded-sm">
      <CardHeader className="pb-3 pt-4 px-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-sm font-medium text-slate-100">Purchase Orders</CardTitle>
      ...
      <CardContent className="px-4 pb-4 space-y-3">
        <div className="grid grid-cols-4 gap-1.5">
          {(["draft", "approved", "fulfilled", "cancelled"] as const).map((k) => (
        ...
        {orders.length === 0 ? (
          <p className="text-sm text-slate-400 py-3 text-center">No purchase orders.</p>
        ) : (
          <div className="space-y-1.5 max-h-64 overflow-y-auto">
            {orders.map((po) => (
              <div key={po.id} className="flex items-center gap-3 px-3 py-2 rounded-sm border border-slate-700/30 bg-slate-800/20">
                <div className="min-w-0 flex-1">
                  <p className="text-xs text-slate-200 truncate">{po.vendorName ?? po.description ?? "PO"}</p>
                  {po.poNumber && <p className="text-[10px] text-slate-500 font-mono">{po.poNumber}</p>}
                </div>
                <span className="text-xs text-slate-300 shrink-0">{fmtCurrency(po.amountMinor, po.currency)}</span>
                <Badge className={`text-[10px] px-1.5 py-0 border shrink-0 ${STATUS_BADGE[po.status] ?? STATUS_BADGE.draft}`}>{po.status}</Badge>
              </div>
            ))}
          </div>
        )}
