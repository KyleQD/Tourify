...
export const AdminUxAuditTaskSchema = z
  .object({
    taskId: z.string().regex(/^ADMUX-\d{4}$/),
    wave: z.string().regex(/^W(?:0\d|1[0-7])$/),
    phase: z.string().min(1),
    workstream: z.string().min(1),
    title: z.string().min(1),
    dependencies: z.array(z.string().regex(/^ADMUX-\d{4}$/)).default([]),
    findingIds: z.array(z.string().regex(/^AUX-[A-Z0-9]+-\d{3}$/)).default([]),
    canonicalBatchId: z.string().regex(/^ADM-B(?:0\d|1[0-4])$/),
    acceptance: z.string().min(1),
    verification: z.string().min(1),
  })
  .strict()

export const AdminUxAuditFindingSchema = z
  .object({
    findingId: z.string().regex(/^AUX-[A-Z0-9]+-\d{3}$/),
...
export const AdminUxAuditWaveSchema = z
  .object({
    wave: z.string().regex(/^W(?:0\d|1[0-7])$/),
    objective: z.string().min(1),
    taskIds: z.array(z.string().regex(/^ADMUX-\d{4}$/)).min(1),
    dependencies: z.array(z.string().regex(/^W(?:0\d|1[0-7])$/)).default([]),
    canonicalBatchIds: z.array(z.string().regex(/^ADM-B(?:0\d|1[0-4])$/)).min(1),
  })
  .strict()

export const AdminUxAuditProgramSchema = z
  .object({
    schemaVersion: z.literal(1),
    sourceId: z.literal("SRC-ADMIN-UX-AUDIT"),
    auditBaseline: z
      .object({
        repository: z.literal("KyleQD/Tourify"),
    ...
    counts: z
      .object({
        findings: z.literal(158),
        tasks: z.literal(54),
        waves: z.literal(18),
        decisions: z.literal(10),
      })
      .strict(),
    statusPolicy: z.string().min(1),
    waves: z.array(AdminUxAuditWaveSchema).length(18),
    tasks: z.array(AdminUxAuditTaskSchema).length(54),
    findings: z.array(AdminUxAuditFindingSchema).length(158),
  })
  .strict()
...
export type AdminUxAuditProgram = z.infer<typeof AdminUxAuditProgramSchema>
