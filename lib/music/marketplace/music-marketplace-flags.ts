...
export type MusicMarketplaceFlagName =
  (typeof MUSIC_MARKETPLACE_FLAG_NAMES)[number];
...
export async function resolveMusicMarketplaceFlags(
  ...
  if (!isLaunchCapabilityAvailable("music_finance_offerings"))
    return { ...DISABLED_MUSIC_MARKETPLACE_FLAGS };

  const { getTrustedMusicWriteClient } = await import(
    "@/lib/music/music-access"
  );
  ...
  for (const row of data as Array<{
    ...
    result[row.key] =
      ...
      (percentage === 100 ||
        Boolean(
          subjectId && stableRolloutBucket(subjectId, row.key) < percentage,
        ));
  }
  return result;
}
