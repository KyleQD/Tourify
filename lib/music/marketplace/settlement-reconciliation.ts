...
export function reconcileSettlement(legs: SettlementLeg[]): SettlementReconciliation {
  const breaks = legs
    ...
    .map(({ currencyOrAsset, expectedMinor, actualMinor }) => ({
      currencyOrAsset,
      expectedMinor,
      actualMinor,
    }))
...
export function positionQuantityBreak(expectedMinor: string, actualMinor: string): boolean {
  return BigInt(expectedMinor) !== BigInt(actualMinor)
}
