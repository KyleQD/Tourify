...
function documentMatchesField(
  document: HiringStoredDocument,
...
function getMatchingDocuments(
  documents: HiringStoredDocument[],
  field: HiringComplianceFieldRule
): HiringStoredDocument[] {
  return documents.filter((document) => documentMatchesField(document, field))
}
...
function getMatchingDocument(
  ...
  return getMatchingDocuments(documents, field).find((document) => document.status !== "rejected")
}

function getDocumentSide(document: HiringStoredDocument): "front" | "back" | null {
  if (document.side === "front" || document.side === "back") return document.side
  const metadataSide = document.metadata?.side
  if (metadataSide === "front" || metadataSide === "back") return metadataSide
  return null
}

function hasCompleteIdDocumentSides(documents: HiringStoredDocument[]): boolean {
  const active = documents.filter((document) => document.status !== "rejected")
  const hasFront = active.some((document) => getDocumentSide(document) === "front")
  const hasBack = active.some((document) => getDocumentSide(document) === "back")
...
function isIdDocumentField(field: HiringComplianceFieldRule): boolean {
  return field.type === "id_document" || field.documentType === "id_document"
}
...
export class HiringComplianceService {
  ...
  evaluateCompliance(input: HiringComplianceCheckInput): HiringComplianceResult {
    ...
    for (const field of requiredFields) {
      const matchingDocuments = getMatchingDocuments(input.documents, field)
      ...
      let hasValue = false
      if (isIdDocumentField(field)) {
        hasValue = hasCompleteIdDocumentSides(matchingDocuments)
      } else if (isDocumentField(field)) {
        hasValue = Boolean(matchingDocument && matchingDocument.status !== "rejected")
      } else {
        hasValue = hasResponseValue(responseValue)
      }
      ...
      if (!hasValue) {
        issues.push({
          ...
          reason: isIdDocumentField(field)
            ? "Front and back government ID photos are required."
            : "Required onboarding item is missing.",
          requiresAdminReview: Boolean(field.requiresAdminReview),
        })
        continue
      }

      const expiredDocument = matchingDocuments.find((document) => document.status === "expired")
      if (expiredDocument) {
        issues.push({
          fieldId: field.fieldId,
          label: field.label,
      ...
      if (field.requiresAdminReview) {
        const reviewDocuments = isIdDocumentField(field)
          ? matchingDocuments.filter((document) => {
              const side = getDocumentSide(document)
        ...
        const allApproved = reviewDocuments.length > 0 && reviewDocuments.every((document) => document.status === "approved")
        if (!allApproved) {
          needsReviewCount += 1
          issues.push({
            fieldId: field.fieldId,
...
function readDocumentSide(metadata: unknown): HiringStoredDocument["side"] {
